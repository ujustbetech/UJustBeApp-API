...
namespace Auth.Service.Manager.Admin.User
{
    public class Select : IDisposable
    {
        ...
        private bool Check_If_User_Exists()
        {
            try
            {
                if (_userInfoService.Check_If_Admin_User_Exists(_userId))
                {
                    return true;
                }
        ...
        private void Get_User_Details()
        {
            try
            {
                _response = _userInfoService.Get_Admin_User_Details(_userId);

                _messages.Add(new Message_Info
                {
                    Message = "ADMIN USER details found Successfully",
