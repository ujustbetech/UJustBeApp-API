...
namespace UJBHelper.Services
{
    public class NotificationService
    {
        ...
        internal bool Check_If_Notification_Inserted()
        {
            ...
            return _notificationQueue.Find(Builders<NotificationQueue>.Filter.In(x => x.status, new[] { "new", "failure" }) & Builders<NotificationQueue>.Filter.Eq(x => x.Type, "Email"))
                      .ToEnumerable()
                    ...
                    if (hours >= 48)
                    {
                        if (_notificationQueue.Find(x => x.userId == userId && x.Event == "Meeting Fee Reminder" && x.status != "new").CountDocuments() != 0 && _notificationQueue.Find(x => x.userId == userId && x.Event == "Meeting Fee Reminder" && x.status == "new").CountDocuments() == 0)
                        {
                            ...
                            if ((DateTime.Now - notificationDate).TotalHours >= 48 && (DateTime.Now - notificationDate).TotalHours < 49)
                            {
                                ...
                                Notification notify_template = new Notification();
                                notify_template = nq.Get_Notification_Template("Membership Fee Reminder");
                                bool isallowed = notify_template.Data.Where(x => x.Receiver == "User").Select(x => x.SMS.isSMSAllowed).FirstOrDefault();
                                if (isallowed)
                                {
        ...
        internal Lead_Email_Details Get_Lead_Mailing_Details(string dealId, int StatusId)
        {
            ...
            res.clientPartneremailId = _businessDetails.Find(x => x.Id == businessId).Project(x => x.BusinessEmail).FirstOrDefault() + "," + _users.Find(x => x._id == res.clientPartnerId).Project(x => x.emailId).FirstOrDefault();
            //   _users.Find(x => x._id == referrerUserId).Project(x => x.emailId).FirstOrDefault();
            ...
            //var statusCode = StatusId; //_lead.Find(x => x.Id == dealId).Project(x => x.dealStatus).FirstOrDefault();
            res.referralCode = _lead.Find(x => x.Id == dealId).Project(x => x.ReferralCode).FirstOrDefault();
            var current_status = _dealStatus.Find(x => x.StatusId == StatusId).Project(x => x.StatusName.ToString()).FirstOrDefault();
        ...
        internal string Get_Receiver_Mobile_Number(string receiver, string userId, string LeadId)
        {
            if (receiver.ToLower().Contains("admin"))
            {
                return _adminUsers.Find(x => x._id == userId).Project(x => x.countryCode.Substring(1) + x.mobileNumber).FirstOrDefault();
            }
            else if (receiver.ToLower().Contains("referred"))
            {
                return _lead.Find(x => x.Id == LeadId).Project(x => x.referredTo.countryCode.Substring(1) + x.referredTo.mobileNumber).FirstOrDefault();
            }
            else
            {
        ...
        internal string Get_Receiver_Email_Id(string receiver, string userId)
        {
            ...
            else
            {
                ...
                {
                    return _users.Find(x => x._id == userId).Project(x => x.emailId).FirstOrDefault() + "," + _businessDetails.Find(x => x.UserId == userId).Project(x => x.BusinessEmail).FirstOrDefault();
                }
        ...
        {
            return _users.Find(x => x._id == userId).Project(x => x.firstName + " " + x.lastName).FirstOrDefault();
        }
        internal string Get_Mentor_Name(string userId)
        {
            var mentorcode= _users.Find(x => x._id == userId).Project(x => x.mentorCode).FirstOrDefault();
            return _users.Find(x => x.myMentorCode == mentorcode).Project(x => x.firstName + " " + x.lastName).FirstOrDefault();
        }

        internal string Get_Receiver_UJBcode(string userId)
        {
            return _users.Find(x => x._id == userId).Project(x => x.myMentorCode).FirstOrDefault();
        }

        internal string Get_bussiness_details(string bussinessId)
        {
            return _businessDetails.Find(x => x.Id == bussinessId).Project(x => x.UserId).FirstOrDefault();
        }

        internal string Get_bussiness_Name(string bussinessId)
        {
            return _businessDetails.Find(x => x.Id == bussinessId).Project(x => x.CompanyName).FirstOrDefault();
        }
