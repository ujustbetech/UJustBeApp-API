...
namespace UJBNotification_SMS.Services
{
    public class NotificationService
    {
        ...
        internal bool Check_If_Notification_Inserted()
        {
          ...
          //  var builder = Builders<NotificationQueue>.Filter.In(x => x.status, new[] { "new", "failure" }) & Builders<NotificationQueue>.Filter.Eq(x => x.Type, "Email" );
           // var filter = builder.Filter.Lt("Age", 40) & builder.Eq("FirstName", "Peter");
            return _notificationQueue.Find(Builders<NotificationQueue>.Filter.In(x => x.status, new[] { "new","failure" }) & Builders<NotificationQueue>.Filter.Eq(x => x.Type, "SMS"))
                      .ToEnumerable()
        ...
        {
            var filter = Builders<NotificationQueue>.Filter.In(x => x.status, new[] { "new", "failure" }) & Builders<NotificationQueue>.Filter.Eq(x => x.Type, "SMS");
